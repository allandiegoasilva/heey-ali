...
describe('HeeyAliHttpEndpointBuilder', () => {
  ...
  describe('when prefix is SYNC', () => {
    it('should build endpoint URL with all params including method', () => {
  ...
  describe('when prefix is REST', () => {
    it('should build endpoint URL with prefix and method in path', () => {
      const builder = new HeeyAliHttpEndpointBuilder(mockCredential);
      const input = {
      ...
      expect(result).toContain('https://api.test.com/resttest.method?');
      expect(result).not.toContain('method=test.method');
      expect(result).toContain('sign_method=sha256');
      expect(result).toContain('app_key=test-app-key');
      expect(result).toContain('param1=value1');
      expect(result).toContain('param2=value2');
    ...
    it('should handle empty params', () => {
      ...
      expect(result).toContain('/resttest.method?');
      expect(result).not.toContain('method=test.method');
      expect(result).toContain('sign_method=sha256');
      expect(result).toContain('app_key=test-app-key');
      expect(result).toContain('sign=');
    ...
    it('should generate valid URL format', () => {
      ...
      expect(() => new URL(result)).not.toThrow();
      expect(result).toMatch(/^https:\/\/.+\/rest.+\?/);
    });
  });
